...
describe "A movie" do
  ...
  it 'is a cult classic' do
    movie = Movie.create!(movie_attributes(total_gross: 50))

    create_reviews(movie, 10)
    expect(movie.flop?).to be true

    create_reviews(movie, 40)
    expect(movie.flop?).to be false

  end

  def create_reviews(movie, number)
    (0..number).each do |index|
      movie.reviews.create!(review_attributes(comment: "comment #{index}", stars: 4))
    end
  end
end